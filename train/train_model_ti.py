...
code_path = '/home/zdf/workspace/Machine_Learning/scene_classification/keras/'
...
def model_train(model, loadweights, isCenterloss, lambda_center=0.01):
    ...
    if not isCenterloss:
        ...
        history = model.fit_generator(train_generator,
                                      steps_per_epoch=62834 // BATCH_SIZE,
                                      epochs=max_Epochs,
...
def save_csv(test_image_path, predict_label):
    image_list = generator_list_of_imagepath(test_image_path)
    save_arr = np.empty((4055, 2), dtype=np.str)
    ...
    save_arr.to_csv('submit_test_ti.csv', decimal=',', encoding='utf-8', index=False, index_label=False)
    print('submit_test_ti.csv have been write, locate is :', os.getcwd())

    # main function
if __name__ == "__main__":
    train_path = code_path + 'train/'
    val_path = code_path + 'validation/'
    test_image_path = 'data1/'
    num_classes = 20
    BATCH_SIZE = 128
    WEIGHTS_PATH = 'best_weights_scene_ti.hdf5'
    max_Epochs = 60
    ...
    model = simple_model
    predict_label = test_image_predict_top_k(model, code_path + test_image_path, train_path, 3)
