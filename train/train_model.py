...
code_path1 = '/home/zdf/workspace/Machine_Learning/scene_classification/zdf/data_prepare/pic/'
code_path2 = '/home/zdf/workspace/Machine_Learning/scene_classification/keras/'
...
# build pipline model
def build_model(out_dims, input_shape=(224, 224, 1)):
    ...
    model = Model(inputs=inputs_dim, outputs=fc3)
    return model
...
def build_centerloss_model(out_dims, feat_dims, input_shape=(128, 128, 1), lambda_center=0.01):
    ...
    fc1 = Dense(4096)(x_flat)
    fc1 = bn_prelu(fc1)
    dp_1 = Dropout(0.5)(fc1)

    fc2 = Dense(4096)(dp_1)
    fc2 = bn_prelu(fc2)
    dp_2 = Dropout(0.5)(fc2)
    
    fc3 = Dense(out_dims)(dp_2)
    fc3 = Activation('softmax')(fc3)

    base_model = Model(inputs=inputs_dim, outputs=fc3)
...
# training model
def model_train(model, loadweights, isCenterloss, lambda_center=0.01):
    lr = LearningRateScheduler(lrschedule)
    mdcheck = ModelCheckpoint(WEIGHTS_PATH, monitor='val_acc', save_best_only=True)
    td = TensorBoard(log_dir=code_path2 + 'image_data/tensorboard_log/')
    ...
    if not isCenterloss:
        ...
        history = model.fit_generator(train_generator,
                                      steps_per_epoch=16623 // BATCH_SIZE,
                                      epochs=max_Epochs,
        ...
        centerloss_model = build_centerloss_model(20, 1024)
        ...
        history = centerloss_model.fit_generator(train_generator,
                                                 steps_per_epoch=16623 // BATCH_SIZE,
                                                 epochs=max_Epochs,
                                                 validation_data=val_generator,
                                                 validation_steps=800 // BATCH_SIZE,
...
def draw_loss_acc(history):
    ...
    plt.ylabel('loss')
    plt.show()
    plt.savefig(code_path2 + 'image_data/image/loss.png', format='png', dpi=300)
    ...
    plt.ylabel('acc')
    plt.show()
    plt.savefig(code_path2 + 'image_data/image/acc.png', format='png', dpi=300)
...
def test_image_predict_top_k(modle, test_image_path, directory, top_k):
    ...
    for image in image_list:
        ...
        label_index = get_label_predict_top_k(img, model, 3)
        label_value_dict = []
...
def save_csv(test_image_path, predict_label):
    image_list = generator_list_of_imagepath(test_image_path)
    save_arr = np.empty((2035, 2), dtype=np.str)
    save_arr = pd.DataFrame(save_arr, columns=['FILE_ID', 'CATEGORY_ID0'])
...
if __name__ == "__main__":
    train_path = code_path1 + 'train/'
    val_path = code_path1 + 'validation/'
    test_image_path = 'data/'
    num_classes = 20
    BATCH_SIZE = 64
    WEIGHTS_PATH = 'best_weights_hanzi1.hdf5'
    max_Epochs = 150
    ...
    #model_history = model_train(simple_model, False, False)
    #print(model_history)
    ...
    model = simple_model
    predict_label = test_image_predict_top_k(model, code_path2 + test_image_path, train_path, 3)

    print("=====csv save=====")
    save_csv(code_path2 + test_image_path, predict_label)
